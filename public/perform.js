...
const clear = ()=>{
 ...
 const fade={
    ...
    playAmbience = ()=> 
            webaudio.loop( "ambience/blue-ridge.mp3"  )
                 .then( player=>{
                     pushMetadata( "ambience") ;
                     ambienceplayer = player;
                     webaudio.setVolume( "ambience", 0); 
               const length = player.buffer.duration,
                     gain   = player.gainNode.gain,
                     start  = player.startTime,
                     clock  = player.context;
                   
                     setInterval( ()=> webaudio.setVolume( "ambience", 
                                    ((1-Math.cos( CYCLES *2*Math.PI * (clock.currentTime-start)/length))/2 )* MAXVOL + MINVOL), 150);
                     }),
       
       
       
    intro=  src=> webaudio.play( src.replace( /(.*\/)[^\/^.]*(\.[^\/^.]*)$/m, "$1intro$2" )),
    outro=  src=> webaudio.play( src.replace( /(.*\/)[^\/^.]*(\.[^\/^.]*)$/m, "$1outro$2" )),


    actionHandler= {
          seekforward:  e=> webaudio.seek("audio",  10),
          seekbackward: e=> webaudio.seek("audio", -10)
          },
    ...
    audio=  content=>    new Promise( (resolve, reject)=> {
            ...
            speak("abort");
                         ...
                         effect(         content  );
                         intro(          content  )
            .then(   p=> console.log( `completed intro of ${content} `, p) )
            .then(  ()=> webaudio.play(  content  ))    
            .then(   p=> console.log( `completed play  of ${content} `, p) )
            .then(  ()=> outro(          content  ))
            .then(   p=> console.log( `completed outro of ${content} `, p) )
            .then(  ()=> popMetadata()    )    
            .catch( ()=> webaudio.play(  content  ))    
            }),
    ...
    speak= content=>    new Promise( (resolve, reject)=> {
        ...
        //  clear();
            ...
            const utt = new SpeechSynthesisUtterance(`o          
                                                      ${content}` );
            utt.pitch = 1;//0.8
            utt.rate  = 1;//0.8;
            webaudio.pause( "audio" );
            utt.addEventListener( "end",  ()=>{
                caption( false );
                webaudio.resume( "audio" );
                popMetadata();
                resolve();
                });
...
//            clear();
    ...
    debug=      content=>  { 
                    ...
                    setTimeout( ()=>fade.out( layer ), 3000);
                    },
...
document.body.addEventListener("pointerdown", e=>webaudio.init().then(  playAmbience ), {once:true});
//  document.body.addEventListener("pointerdown", e=> setTimeout( playAmbience, 5000 ), {once:true});
...
const setMetadata= 
    ...
    metaStack= [ null ], 
    pushMetadata=  name=> { setMetadata( name );   metaStack.push( name );   },
    popMetadata =    ()=>   setMetadata(           metaStack.pop()           ),
