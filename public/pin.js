...
export default  context =>{
    ...
    const pinStyle={
        ...
        story:   { color:  "red",     opacity: 1.00,  showZones: false },
        history: { color:  "blue",    opacity: 1.00,  showZones: false },
        guide:   { color:  "blue",    opacity: 1.00,  showZones: false },
        music:   { color:  "orange",  opacity: 1.00,  showZones: false },
        nav:     { color:  "green",   opacity: 0.20,  showZones: false },
...
//      map.handle.on(                'click',     self.open   );
    ...
    render: dnaUpdate =>{
        ...
        if( app.editEnabled)//   if( pinStyle[status].showZones ) 
                self.children.forEach( z=> z.render() );
        else  { 
            let classes= self.children.map( z=> z.class );
            status= classes.includes("story")? "story" :
                    classes.includes("guide")? "guide" :
                    classes.includes("music")? "music" :
                    classes.includes("nav"  )? "nav"   : status;
            }
        ...
        map.handle  = L.marker( 
                            ...
                            {icon:pinStyle[ status ??"nav}" ].icon ?? pinStyle.default.icon })
                       .addTo( map.base )
                       .bindTooltip(dna.id,  {
    ...
    detectable: ()=>     !self.parent.nextPinRequired 
               || self == self.parent.nextPinRequired,   
