...
export default  context =>{
    ...
    const dom ={            // all DOM nodes that display tour components
        ...
        unit: false,        // contains interactive html representation of this zone
    ...
    const map ={           // everything that displays the pin on the map   
        ...
        arc: false   // the interactive visual display of the actual zone footprint
        }

    var handling = false;
    var radiusSq = false; // this zone's radius (in latlng) squared,for fast trigger testing (no square root needed)
    ...
    const  statusStyle = {
        ...
        rim:           { smoothFactor:.3, fillColor:false,    color:"#080",   weight:3,  interactive:false, dashArray:"6,6" },
        };
...
update: e=>{// zone only----    
   ...
   //         zone angles   in Turns   ( 1 turn = 360 degrees = 2PI radians
    ...
    render: update =>{
        ...
        if( update?.radius || !radiusSq  ) radiusSq = m2l( dna.radius )**2;
...
*/
    ...
    detect: probe =>{  // in LatLan space
      ...
      //  if( probe=="start" && dna.next=="start" )        return true;          
        if( requiredPin &&  ( requiredPin.id !=  self.parent.dna.id ) ) return false;
    ...
    trigger: probe =>  {
        if( dna.status!="spent"  &&  self.detect( probe )) {
            ...
            // requiredPin =  false;
            // if( typeof dna.next == "string" ){
            //     let pinName= dna.next.split(",")[0];
            //     tour.setNext( pinName );  
            //     // if( dna.next.includes("modal") )        requiredPin= true;
            //     // if( dna.next.includes("required") )     requiredPin= true;
            //     // if( dna.modal )                         requiredPin= true;
            //     // if( pinName.toLowerCase()=="start")     requiredPin= true;

            tour.setNext(  dna.next  );  
            requiredPin =  dna.modal? tour.getPin( dna.next ): false;
            console.log( `active pin:${self.parent.id}  next pin:${dna.next}  required pin:${requiredPin?.id}`   )
            // }
            }
        }
    };
