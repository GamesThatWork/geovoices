...
export  default config=> new Promise( (resolve, reject) => {
    ...
    const construct= ()=>{
     ...
     const injectColorClasses = ()=>{
        ...
        oldColors?.remove();  //actually unreachable 
        ...
        let weavelayer= container.querySelector("div.portrait");
        const rectSize= ( (weavelayer.clientWidth||(1920*.45))/cols )/ 1.38 ; //trial & error
    
        const pitch = { x:scale*weavelayer.clientWidth, y:scale*weavelayer.clientHeight };     
        const id = scale==1?"weave" :`weave_${i}_${j}`
        
        if( !weavelayer.querySelector(`svg#${id}` ))
             weavelayer.insertAdjacentHTML( "beforeend",`<svg id="${id}" class="fxsvg" width="100%" height="100%" pointer-events="none" version="1.1" xmlns="http://www.w3.org/2000/svg"></svg>`);
  //         else weavelayer.querySelector(`svg#${id}` ).innerHTML="";
        svg= weavelayer.querySelector(`svg#${id}` );

        svg.innerHTML=`            <g id="g-${id}" transform="translate(${pitch.x*i} ${pitch.y*j}) scale(${scale})"  pointer-events="all" >
        ...
        rectgroup =  weavelayer.querySelector(`#g-${id}` );
